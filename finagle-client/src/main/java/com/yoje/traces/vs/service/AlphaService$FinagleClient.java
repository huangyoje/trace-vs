...
public class AlphaService$FinagleClient implements AlphaService.FutureIface {
    ...
    try {
      if (msg.type == TMessageType.EXCEPTION) {
        ...
        if (exception instanceof SourcedException) {
          if (this.serviceName != "") ((SourcedException) exception).serviceName_$eq(AlphaService$FinagleClient.this.serviceName);
        }
        throw exception;
      } else {
  ...
  protected class __Stats {
    ...
    public __Stats(String name) {
      StatsReceiver scope = AlphaService$FinagleClient.this.scopedStats.scope(name);
    ...
    Future<FinagleResponse> rv = this.service.apply(encodeRequest("invoke", new Invoke.Args(request))).flatMap(new Function<byte[], Future<FinagleResponse>>() {
      ...
      public Future<FinagleResponse> apply(Throwable t) {
        if (t instanceof SourcedException) {
          ((SourcedException) t).serviceName_$eq(AlphaService$FinagleClient.this.serviceName);
        }
