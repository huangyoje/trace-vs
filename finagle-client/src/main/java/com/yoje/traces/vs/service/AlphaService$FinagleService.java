...
public class AlphaService$FinagleService extends com.twitter.finagle.Service<byte[], byte[]> {
  final private AlphaService.FutureIface iface;
  final private TProtocolFactory protocolFactory;

  public AlphaService$FinagleService(final AlphaService.FutureIface iface, final TProtocolFactory protocolFactory) {
    this.iface = iface;
    this.protocolFactory = protocolFactory;

    addFunction("invoke", new Function2<TProtocol, Integer, Future<byte[]>>() {
      public Future<byte[]> apply(TProtocol iprot, final Integer seqid) {
        try {
          Invoke.Args args = Invoke.Args.decode(iprot);
          iprot.readMessageEnd();
          Future<FinagleResponse> result;
          try {
            result = iface.invoke(args.request);
          } catch (Throwable t) {
            result = Future.exception(t);
          }
          return result.flatMap(new Function<FinagleResponse, Future<byte[]>>() {
            public Future<byte[]> apply(FinagleResponse value){
              return reply("invoke", seqid, new Invoke.Result.Builder().success(value).build());
            }
          ...
          try {
            iprot.readMessageEnd();
            return exception("invoke", seqid, TApplicationException.PROTOCOL_ERROR, e.getMessage());
